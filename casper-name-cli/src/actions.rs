...
use casper_name_contracts::contracts::controller::{Controller, ControllerHostRef, ControllerInitArgs};
use casper_name_contracts::contracts::registrar::{Registrar, RegistrarHostRef, RegistrarInitArgs, CONTROLLER_ROLE};
...
pub fn deploy_all() {
    ...
    let controller = ControllerHostRef::deploy(&env, ControllerInitArgs {
...
pub fn set_config() {
    ...
    env.set_gas(1_000_000_000);
    contracts.registrar.grant_role(&CONTROLLER_ROLE, contracts.controller.address());
    ...
    env.set_gas(1_000_000_000);
    contracts.registrar.set_default_resolver(*contracts.resolver.address());
}
...
pub fn controller_buy(name: &str) {
    ...
    let names = vec![NameMintInfo::new(name, env.get_account(0), voucher_expiration)];
    ...
    contracts.controller.with_tokens(amount).buy(payment_voucher, signature);
}