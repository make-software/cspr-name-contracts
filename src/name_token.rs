...
impl NameToken {
    ...
    //     if label.is_empty() {
    //         self.env().revert(NameTokenError::EmptyLabel);
    //     }
...
impl NameToken {
    ...
    pub fn compute_namehash(&self, label: &str) -> TokenHash {
        let hash = self.env().hash(label);
        hex::encode(hash)
    }

    pub fn require_future_expiration_date(&self, expiration: u64) {
        if expiration < self.env().get_block_time() {
            self.env().revert(NameTokenError::PastExpirationDate);
        }
    }

    pub fn require_token_minted(&self, token_hash: &TokenHash) {
        if !self.token_exists(token_hash) {
            self.env().revert(NameTokenError::SLDDoesNotExist);
        }
    }

    pub fn token_exists(&self, token_hash: &TokenHash) -> bool {
        self.token.token_exists_by_hash(token_hash)
    }

    pub fn is_token_expired(&self, token_hash: &TokenHash) -> bool {
        self.expiration(token_hash) < self.env().get_block_time()
    }

    pub fn burn_single(&mut self, token_hash: TokenHash, burner: Address) {
        self.token.burn_token_unchecked(token_hash, burner);
    }

    pub fn mint_single(&mut self, to: Address, label: String, token_hash: &TokenHash, expiration: u64) {
    ...
    pub fn assert_minter_role(&self) {
        self.access_control
            .check_role(&MINTER_ROLE, &self.env().caller());
    }

    pub fn assert_operator_role(&self) {
        self.access_control
            .check_role(&OPERATOR_ROLE, &self.env().caller());
    }

    pub fn expiration(&self, token_hash: &TokenHash) -> u64 {
        let metadata = self.get_metadata(token_hash);
        metadata.expiration
    }

    pub fn set_expiration(&mut self, token_hash: &TokenHash, expiration: u64) {
    ...
    pub fn get_metadata(&self, token_hash: &TokenHash) -> SLDMetadata {
        let metadata = self
            .token
